...
{
QStringList sl;sl.reserve(from.size());for(const auto&s:from)sl.push_back(RustStrToQString(s));return QVariant(std::move(sl));
}
...
{
if(!from.canConvert<QString>())return false;QString s=from.value<QString>();result=QStringToRustString(s);return true;
}


bool inlineCppFn_TraitImpl_TryFrom_ref_QVariant_for_Vec_String_try_from(const QVariant& from, rust::Vec<rust::String>& result)
{
if(!from.canConvert<QStringList>())return false;QStringList sl=from.value<QStringList>();result=QStringListToRustStringList(sl);return true;
}
