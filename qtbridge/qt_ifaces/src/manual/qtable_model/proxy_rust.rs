...
use super::proxy_cpp_bridge::{QTableModelProxyCpp, ffi};
...
impl QRustProxy for QTableModelProxyRust {
    ...
    fn new(rust_obj: &Rc<RefCell<dyn QTableModelAdapter>>, construct: ConstructionMode, on_drop: fn(rust_obj: *const u8)) -> *mut Self {
        ...
        unsafe{ (*raw_self).cpp_proxy = match construct {
            ConstructionMode::AtAddress(addr) => {
                ffi::create_qtable_model_proxy_cpp_at( addr, raw_rust_obj.cast(), raw_self)
            }
            ConstructionMode::Strong | ConstructionMode::Weak => {
                ffi::create_qtable_model_proxy_cpp(raw_rust_obj.cast(), raw_self)
            }
        }};
        raw_self
    }
