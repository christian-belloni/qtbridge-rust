...
pub fn generate_qt_types_getters_code() -> Result<TokenStream, String> {
    ...
    let code = quote! {
        pub fn get_non_generic_types() -> [QtNonGenericType; #non_generic_count] {
            [
                #(#non_generic_vec),*
            ]
        }
        pub fn get_generic_types() -> [QtGenericTypeWithoutArgs; #generic_count] {
            [
                #(#generic_vec),*
            ]
        }
        pub fn get_monomorphed_types(generics: &[QtGenericTypeWithoutArgs], non_generics: &[QtNonGenericType]) -> [QtMonomorphedType; #monomorphed_count] {
            use crate::type_registry::PrimitiveType;
            [
                #(#monomorphed_vec),*
            ]
        }
        pub fn get_alias_to_monomorphed_types() -> [QtAliasToMonomorphedType; #alias_count] {
            [
                #(#alias_vec),*
            ]
        }
