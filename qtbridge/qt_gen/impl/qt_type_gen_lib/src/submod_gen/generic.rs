...
use qt_gen_common_no_types::signature_utils::get_arg_ident;
use qt_gen_common_no_types::type_registry::QtType;
use qt_gen_common_no_types::type_to_string::{angle_bracketed_generic_arguments_to_string, path_to_string};
use qt_gen_common_no_types::type_utils::{are_all_args_generic_idents, get_angle_bracketed_generic_arguments_of_last_path_segment, get_ident_of_last_path_segment};
...
impl SubmoduleGenerator for GenericSubmoduleGenerator {
    ...
    fn register_type(&self) -> syn::Result<()> {
        let struct_ = self.structure();
        QtType::add_generic(struct_.ident().to_string(), self.path_before_name()?, struct_.generics().list().into());
